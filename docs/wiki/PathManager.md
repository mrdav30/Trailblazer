...
> A temporary legacy chart-management surface remains on the service for
> non-graph callers. It does not feed unified A*/Flow routing and is scheduled
> for removal by the repository tracker. New code should use the operation
...
`FlushPendingGridChanges()` drains pending GridForge event rebuild work for the
context. Physical add/remove/blockage and grid lifecycle changes are composed
before semantic overlay refresh for the same publication boundary.
