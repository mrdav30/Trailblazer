...
public sealed class NavigationRayDeterminismMatrixTests
{
    private readonly ITestOutputHelper _output;

    public NavigationRayDeterminismMatrixTests(ITestOutputHelper output) =>
        _output = output;
    ...
    public void CanonicalCase_ShouldMatchCheckedInDigest(
        string caseName,
        string expectedDigest)
    {
        ...
        _output.WriteLine($"NAVIGATION_RAY_CANONICAL={caseName}|{canonical}");
        _output.WriteLine($"NAVIGATION_RAY_DIGEST={digest} CASE={caseName}");

        Assert.Equal(expectedDigest, digest);
    }

    [Fact]
    public void CanonicalHashes_ShouldBeCultureInvariant()
    {
    ...
    private static string BuildCanonical(string caseName) => caseName switch
    {
        ...
        _ => throw new InvalidOperationException($"Unknown navigation-ray digest case '{caseName}'.")
    };
