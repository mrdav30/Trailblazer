...
`Move` and `Fall` are the required core locomotions. `Platform`, `Jump`, `Slide`, `Swim`, and `Fly` are optional and can be installed or removed through a locomotion profile.
