...
internal sealed class NavigationAStarAdmissionGate : IDisposable
{
        ...
        NavigationQueryLimits limits)
    {
        ...
        SwiftThrowHelper.ThrowIfArgument(
            limits.MaxConcurrentAStarQueries <= 0,
            nameof(limits),
        ...
        _queries = new NavigationAStarQueryWork[limits.MaxConcurrentAStarQueries];
        _leases = new NavigationWorldGraphLease[limits.MaxConcurrentAStarQueries];
        _reservations = new NavigationAStarPayloadReservation[limits.MaxConcurrentAStarQueries];
    ...
    private bool FitsBatchEnvelope(PathQueryBatch batch)
    {
        ...
        if (count < 0 || count > _limits.MaxBatchItems)
            return false;
        if (count == 0)
            return false;
        long descriptorBytes = checked((long)count * PathQueryBatchItem.LogicalRetainedBytes);
        for (int i = 0; i < count; i++)
        {
            PathQuery query = batch.Items[i].Query;
            descriptorBytes = AddStringPayload(descriptorBytes, query.Start.MapId);
            descriptorBytes = AddStringPayload(descriptorBytes, query.End.MapId);
            descriptorBytes = AddStringPayload(descriptorBytes, query.AreaPolicy.PolicyId);
        }
        return descriptorBytes <= _limits.MaxBatchDescriptorBytes;
    }

    private bool FitsSingleEnvelope(PathQuery query)
    {
        long descriptorBytes = PathQueryBatchItem.LogicalRetainedBytes;
        descriptorBytes = AddStringPayload(descriptorBytes, query.Start.MapId);
        descriptorBytes = AddStringPayload(descriptorBytes, query.End.MapId);
        descriptorBytes = AddStringPayload(descriptorBytes, query.AreaPolicy.PolicyId);
        return descriptorBytes <= _limits.MaxBatchDescriptorBytes;
    ...
    private static long AddStringPayload(long bytes, string? value) =>
        value == null
            ? bytes
            : checked(bytes + ((long)value.Length * sizeof(char)));

    private NavigationAStarQueryStatus BeginCore(
        int count,
        out NavigationAStarBatchWork work)
    {
        int candidateCount = Math.Min(count, _queries.Length);
        ...
        for (int i = 0; i < admitted; i++)
        {
            ...
            _descriptors[i].IsAdmitted = true;
        ...
        for (int i = admitted; i < count; i++)
        {
            _descriptors[i].Status = NavigationAStarQueryStatus.CapacityExceeded;
            _descriptors[i].IsAdmitted = false;
        }
    ...
    internal int GetCount(NavigationAStarBatchWork work)
    {
        lock (_sync)
        {
            EnsureActive(work);
    ...
    internal bool IsComplete(NavigationAStarBatchWork work)
    {
        lock (_sync)
        {
            EnsureActive(work);
            return _nextPublication >= _activeAdmittedCount;
        }
    }
