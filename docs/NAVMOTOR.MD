...
`Move` and `Fall` are the required core locomotions. `Platform`, `Jump`, `Slide`, and `Swim` are optional and can be installed or removed through a locomotion profile.
