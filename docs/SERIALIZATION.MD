...
The core now also supports stable external links through `ChronicleContext` and `RecordLinks`, but the current Trailblazer runtime does not need direct `RecordLinks` usage in its core navigation branch after the animation and platform APIs were tightened around explicit host bindings and sampled value snapshots.
...
- it now supports immediate and deferred stable links, but that capability is currently more relevant to the shared Chronicler layer and future addons/frameworks than to Trailblazer's core runtime
...
3. Add more targeted tests around partial loads, disabled locomotions, backward-compatible defaults, failed request rehydration, unresolved deferred links, and transport-parity rebuild behavior.
